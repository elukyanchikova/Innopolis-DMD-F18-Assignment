...
public class DatabaseQueries {
	...
	public void FillSample(long requestedDate) {
		sample.connect();
		sample.clear();

		DataGenerator dataGenerator = new DataGenerator();
		GeneratedData metaData = dataGenerator.generateData(5, 30, 40, 2, 2, 2, 124, 120, 3, 12, 123, 110, requestedDate);
	...
	public void Query03(long requestedDate) {
		try {
			...
			int coeff = (100 / result0.getInt(1));
	...
	public ResultSet Query04(long requestedDate) {
		try {
			...
			return sample.executeQuery("SELECT * FROM query4");
		} catch (SQLException e) {
			e.printStackTrace();
			return null;
		}
		...
		try {
			...
			for (ResultSet result : res) {
				String insert = "INSERT INTO query9 (WID, part_name, number_of_parts)"
						+ "VALUES (" + result.getInt(1) + ", '" + result.getString(2) + "',  " + result.getInt(3) / weeksNumber + " )";
				sample.execute(insert);
			}

			return sample.executeQuery("SELECT * FROM query9");
